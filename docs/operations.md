...
`radian-tool-lint-scope` is centered on cli tools work in Python. The technical target is: Package a Python local lab for lint analysis with round-trip fixtures, lossless normalization checks, and documented operating limits.
...
The score is deterministic. The current threshold is 173. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 4.
